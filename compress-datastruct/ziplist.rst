...
一个 ziplist 可以包含多个节点（entry），每个节点可以保存一个长度受限的字符数组（不以 ``\0`` 结尾的 ``char`` 数组）或者整数，
包括：
...
本章先介绍 ziplist 的组成结构，以及 ziplist 节点的编码方式。
...
作为例子，
以下是个长度为 ``1`` 字节的 ``pre_entry_length`` 域，
...
而以下则是个长度为 5 字节的 ``pre_entry_length`` 域，
...
然后将 ``p`` 加上 ``e1-size`` ，
就将指针后移到了下一个节点 ``e2`` 。。。
如此反覆，直到 ``p`` 遇到 ``ZIPLIST_ENTRY_END`` 为止，
这样整个 ziplist 就遍历完了：
...
这就将指针移动到了前一个节点 ``eN-1`` 。。。
如此反覆，直到 ``p`` 遇到 ``ZIPLIST_ENTRY_HEAD`` 为止，
这样整个 ziplist 就遍历完了。
